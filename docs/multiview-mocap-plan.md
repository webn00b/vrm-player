...
Важно: `source` сейчас в `CanonicalMotionClip` типизирован ограниченным списком. Для нового значения есть два варианта:
...
Сначала сделать то же, что делает текущий `video_to_motion_mediapipe.py`, но отдельно для каждой камеры:
...
На этом этапе полезно сохранить debug JSON:
...
- при необходимости расширить `CanonicalMotionClip['source']`;
...
1. Создать `video_pair_to_motion_mediapipe.py` с CLI и чтением двух видео.
2. Вынести общую MediaPipe extraction логику из `video_to_motion_mediapipe.py`, чтобы не дублировать landmark mapping.
3. Добавить ручной sync offset.
4. Реализовать rough-orthogonal fusion с настройками depth axis/scale.
5. Записать итоговый `.multiview.motion.json`.
6. Прогнать через текущий app import.
7. Добавить debug report.
8. Добавить базовые tests.
9. Обновить `docs/offline-mocap-import.md`.
10. Только после этого думать о настоящей calibration/triangulation.

## Первые конкретные задачи

- [ ] Добавить `tools/offline_mocap/video_pair_to_motion_mediapipe.py`.
- [ ] Вынести helper-функции из `video_to_motion_mediapipe.py` в общий модуль `tools/offline_mocap/mediapipe_common.py`.
- [ ] Добавить `source: "multiview"` в `CanonicalMotionClip`.
- [ ] Добавить обработку `multiview` в `inferSource()`.
- [ ] Добавить sample command в `docs/offline-mocap-import.md`.
- [ ] Создать один локальный тестовый pair clip и прогнать до `.motion.json`.
