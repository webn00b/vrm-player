...
Если нужно понять, как меряются размеры человека и как устроен путь «видео → анимация» (сначала простыми словами, потом технически), откройте [sizing-and-pipeline.md](./sizing-and-pipeline.md).
